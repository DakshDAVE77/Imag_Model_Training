...
def finetune_lora(images, captions, output_dir, num_epochs=40, lr=1e-4, rank=16):
    ...
    # Force VAE to float32 to prevent SD1.5 VAE NaN issues
    ...
    for i in range(0, len(images), 2):
        img_t = torch.tensor(np.array(images[i:i+2]), dtype=torch.float32).to(device)
        with torch.no_grad():
            lat = vae.encode(img_t).latent_dist.sample()
            all_latents.append((lat * vae.config.scaling_factor).cpu().to(torch.float32))
            tok = tokenizer(captions[i:i+2], padding='max_length',
                           max_length=tokenizer.model_max_length, truncation=True,
                           return_tensors='pt').to(device)
            all_text_embs.append(text_encoder(**tok)[0].cpu().to(torch.float32))
        del img_t, lat, tok
    ...
    print(f'Injecting SD 1.5 LoRA (rank={rank}) into all attention layers...')
    lora_params = []

    for name, module in unet.named_modules():
        if not (hasattr(module, 'weight') and isinstance(module, torch.nn.Linear)):
            continue
        # SD 1.5 attention names usually end in to_q, to_k, to_v, to_out.0
        ...
        module.forward = make_forward(
            ...
            module.bias.detach() if getattr(module, 'bias', None) is not None else None
    ...
    unet.eval()  # we only train LoRA, base model in eval
    best_loss = float('inf')
    t0 = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for idx in perm:
            ...
            # Add noise
            noisy_latents = noise_scheduler.add_noise(lat, noise, timesteps)

            # removed autocast for stability
            model_pred = unet(noisy_latents, timesteps, encoder_hidden_states=emb).sample
        ...
        if avg < best_loss:
            ...
            state = {name: {'lora_a': m.lora_a.data.cpu(),
                     ...
                     if hasattr(m, 'lora_a')}
            torch.save(state, os.path.join(output_dir, 'best_sd_lora.pt'))
    ...
    state = {name: {'lora_a': m.lora_a.data.cpu(),
             ...
             for name, m in unet.named_modules()
             if hasattr(m, 'lora_a')}
    torch.save(state, os.path.join(output_dir, 'final_sd_lora.pt'))
...
if __name__ == '__main__':
    print('=' * 55)
    print('SD 1.5 LoRA Training | 50 epochs | Full dataset')
    print('RTX 4090 | Rank 16 | All attention layers')
    print('=' * 55)
